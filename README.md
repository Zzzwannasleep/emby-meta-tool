...
- **实时预览 / 手动输入切换**：在“预览 / 手动输入”卡片可随源自动展示当前选中条目的标题、年份、ID、语言、预计集数等；切换到手动模式时自动出现可编辑的各字段输入框。
...
## 👀 预览 / 手动输入
- 非手动模式：右侧卡片实时展示当前选择的基础元信息（标题/原名/年份/ID/语言/集数估算、NFO 命名模式），便于生成前确认。
- 手动模式：同一区域自动切换为输入表单，可逐项填写元信息与季/集结构，再驱动预览/生成。
